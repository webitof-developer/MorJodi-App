...
const Nearby = () => {
  ...
  const fetchData = async (pageNum = 1) => {
    if (token) {
      await dispatch(fetchProfiles({ profileType: 'nearby', page: pageNum }));
    }
  };
  useEffect(() => {
    if (token) {
      setPage(1);
      dispatch(fetchProfiles({ profileType: 'nearby', page: 1 }));
    }
  }, [token, dispatch]);
  ...
  const loadMore = () => {
    ...
    const totalPages = pagination?.nearby?.totalPages || 1;
    if (page < totalPages) {
    ...
    return (
      ...
      >
        ...
        <Text style={{ fontSize: 16, color: '#555', marginTop: 10 }}>
          {i18n.t('nearby2.empty', { lng: language })}
        </Text>
      </View>
    );
  }
